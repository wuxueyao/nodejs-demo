...
function select(req,res){
  ...
  log(pathname)
  var pathString = url.parse(req.url).query;
  var item = qs.parse(pathString);
  var file = __dirname;
  var fileobj = [];
  if(item.chapterId && pathString != null){
    var inner = JSON.stringify(chapterList[item.chapterId-1]);
    res.setHeader('Content-Length',Buffer.byteLength(inner));
  ...
  if(pathname === '/list/'){
    var num = JSON.stringify(chapterList);
    res.setHeader('Content-Length',Buffer.byteLength(num));
    res.setHeader('Content-Type','text/plain;charset="utf8"');
  ...
  switch(pathname){
    ...
    case '/listmanager/':
      file += '/list.html';
      break;
    case '/addChapter.html':
      file += '/addChapter.html';
      break;
    default:
      var fileString = pathname.split('?')[0];
      fileobj = fileString.split('/');
      for(var i=2;i<fileobj.length;i++){
        var name = '/'+fileobj[i];
        file += name;
      }
      break;
  ...
  fs.readFile(file,(err,data)=>{
    if(err){
      ...
      log(file.split('.')[1]);
      if(file.split('.')[1] === 'js' || file.split('.')[1] === 'html' || file.split('.')[1] === 'css'){
